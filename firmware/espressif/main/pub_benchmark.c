...
// create a UInt64 publisher with chosen QoS and topic
    ...
    for (int i = 0; i < SYNC_TRIALS; i++) {
        struct timeval tv;
        gettimeofday(&tv, NULL); // Synchronized timestamp (wallclock)
        uint64_t now_us = ((uint64_t)tv.tv_sec * 1000000ULL) + (uint64_t)tv.tv_usec;
        latency_msg.data = now_us;
        local_us[i] = now_us;

        uint64_t t0 = esp_timer_get_time(); // Un-synchronized timestamp (monotonic)
        RCSOFTCHECK(rcl_publish(&publisher, &latency_msg, NULL), "publish latency");
